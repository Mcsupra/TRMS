...
let addRow = function (myRequest) {
    ...
    if(myRequest.benco != true){
        reqIdCol.innerHTML = myRequest.reqId;
    }else{
        reqIdCol.innerHTML = `<a href="reimbursement-approval/${myRequest.reqId}">`+myRequest.reqId+`</a>`; 
    }
    ...
    if (myRequest.currentStatus == 'DENIED'){
        ...
        '<form action="approve-request" method="POST">'
            ...
            +'<input name="title"class="hide" id="title" value="BENCO"></input>'
